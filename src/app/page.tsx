...
export default async function DashboardPage({
  ...
  const [
    ...
    supabase
      .from('visits')
      .select('id, checked_in_at, checked_out_at, member_id, membership_id, visit_type, children_present, adults_count, children_count, booking_id, paid_at, members(name), memberships(sessions_remaining, membership_types(name, sessions, price)), bookings(type)')
    ...
    supabase
      .from('members')
      .select('id, name, phone, family_id, families(name), children'),
    supabase
      .from('bookings')
    ...
    supabase
      .from('bookings')
      .select('id, type, title, start_time, end_time, guests, guest_adults, guest_children, child_name, executed_at, member_id, members(name)')
      .eq('date', selectedDate)
