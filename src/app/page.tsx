...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase
      .from('bookings')
      .select('id, title, type, start_time, end_time, status, child_name, guests, member_id, members(name)')
      .eq('date', todayStr)
  ...
  const todayBookings = (todayBookingsData ?? []) as any[]

  // Children with birthday today
  ...
  for (const member of (allMembers ?? []) as any[]) {
    for (const child of (member.children ?? []) as any[]) {
      ...
      const booking = todayBookings.find((b: any) => b.type === 'birthday' && b.member_id === member.id) ?? null
      todayBirthdays.push({
        name: child.name,
